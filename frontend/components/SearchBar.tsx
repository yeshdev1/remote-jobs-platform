    ...
    <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
      <div className="relative group">
        ...
        <button
          ...
          className="absolute inset-y-0 right-0 px-6 bg-gradient-to-r from-slate-600 to-slate-800 text-white rounded-r-xl hover:from-slate-700 hover:to-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-400/50 focus:ring-offset-2 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-medium disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
        >
          {isSearching ? (
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-slate-500/20 to-slate-700/20 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 blur-xl"></div>
      </div>
    </form>
  );
}

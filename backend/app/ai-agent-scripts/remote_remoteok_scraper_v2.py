...
def extract_url_from_job_data(job):
    ...
    if 'apply_url' in job:
        ...
        # Construct URL from job ID for RemoteOK
        return f"https://remoteok.io/remote-jobs/{job['id']}"
    return None
...
def main():
    ...
    for source in JOB_SOURCES:
        ...
        if json_data:
            ...
            if job_listings:
                ...
                new_jobs, skipped_count = filter_jobs_by_timestamp(job_listings, "RemoteOK")
                ...
                analyzed_jobs = analyze_with_o1_mini(job_listings=new_jobs)
    ...
    if not all_jobs and total_skipped > 0:
        print_scraping_summary(total_skipped, 0, 0, "RemoteOK")
    ...
    print_scraping_summary(total_skipped, len(all_jobs), inserted_count, "RemoteOK")

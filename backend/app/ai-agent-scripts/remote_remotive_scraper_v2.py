...
def extract_url_from_job_data(job):
    ...
    if 'url' in job:
        return job['url']
    elif 'job_slug' in job and 'company_name' in job:
        # Construct URL from job data for Remotive
        return f"https://remotive.com/remote-jobs/{job['company_name'].lower().replace(' ', '-')}/{job['job_slug']}"
    elif 'id' in job:
        return f"https://remotive.com/remote-jobs/{job['id']}"
    return None
...
def extract_job_listings(json_data):
    ...
    if not json_data or 'jobs' not in json_data:
        return []
    ...
    for i, job in enumerate(jobs):
        ...
        <div class="job-listing">
            ...
            <div class="candidate-required-location">{job.get('candidate_required_location', '')}</div>
            ...
            <div class="job-slug">{job.get('job_slug', '')}</div>
        </div>
        """
        ...
        job_data = {
            ...
            'original_job_data': job  # Store original job data for URL extraction
        }
...
def fetch_job_page(url):
    """Fetch the job listing page and return the JSON content"""
    try:
...
def main():
    ...
    for source in JOB_CATEGORIES:
        ...
        if json_data:
            ...
            if job_listings:
                ...
                new_jobs, skipped_count = filter_jobs_by_timestamp(job_listings, "Remotive")
    ...
    if not all_jobs and total_skipped > 0:
        print_scraping_summary(total_skipped, 0, 0, "Remotive")
    ...
    print_scraping_summary(total_skipped, len(valid_jobs), inserted_count, "Remotive")
